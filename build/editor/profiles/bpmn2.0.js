...
f.on("keyup",function(Y,k){this.editDirectly(t,Y.getValue())
}.bind(this));
N=new Ext.Editor(f)
}else{var D=new Ext.form.TextField({allowBlank:p.optional(),msgTarget:"title",maxLength:p.length()});
D.on("keyup",function(k,Y){this.editDirectly(t,k.getValue())
}.bind(this));
D.on("blur",function(k){if(!k.isValid(false)){this.updateAfterInvalid(t)
}}.bind(this));
D.on("specialkey",function(k,Y){if(!k.isValid(false)){this.updateAfterInvalid(t)
}}.bind(this));
N=new Ext.Editor(D)
}break;
case ORYX.CONFIG.TYPE_BOOLEAN:var n=new Ext.form.Checkbox();
n.on("check",function(Y,k){this.editDirectly(t,k)
}.bind(this));
N=new Ext.Editor(n);
break;
...
z.on("keyup",function(k,Y){this.editDirectly(t,k.getValue())
}.bind(this));
N=new Ext.Editor(z);
break;
...
var y=[];
v.each(function(k){if(k.value()==C){C=k.title()
}if(k.refToView()[0]){G=true
}y.push([k.icon(),k.title(),k.value()]);
X.push({name:k.title(),icon:k.icon()})
});
...
o.on("select",function(Z,k,Y){this.editDirectly(t,Z.getValue())
}.bind(this));
N=new Ext.Editor(o);
break;
...
e=new Ext.form.ComboBox({editable:false,typeAhead:true,triggerAction:"all",transform:l,lazyRender:true,msgTarget:"title",width:b})
}else{if(g==ORYX.CONFIG.TYPE_DYNAMICCHOICE){var f=this.items[c].items();
var l=ORYX.Editor.graft("http://www.w3.org/1999/xhtml",m,["select",{style:"display:none"}]);
var k=new Ext.Template('<option value="{value}">{value}</option>');
f.each(function(s){var q=ORYX.EDITOR.getSerializedJSON();
var r=jsonPath(q.evalJSON(),s.value());
if(r){if(r.toString().length>0){for(var p=0;
p<r.length;
p++){var t=r[p].split(",");
for(var o=0;
o<t.length;
o++){if(t[o].indexOf(":")>0){var n=t[o].split(":");
k.append(l,{value:n[0]})
}else{k.append(l,{value:t[o]})
}}}}}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"No data available for property.",title:""})
}});
e=new Ext.form.ComboBox({editable:false,typeAhead:true,triggerAction:"all",transform:l,lazyRender:true,msgTarget:"title",width:b})
...
}}],clicksToEdit:1,listeners:{beforeedit:function(E){if(E.column!=8){return true
}var F=Ext.get("notificationsBodyEditorWindow");
if(!F){var G=new Ext.Window({id:"notificationsBodyEditorWindow",modal:true,collapsible:false,fixedcenter:true,shadow:true,proxyDrag:true,autoScroll:true,autoWidth:true,autoHeight:true,bodyBorder:false,closable:true,resizable:true,items:[{xtype:"panel",html:"<p class='instructions'>Enter Notification body message.</p>"},{xtype:"textarea",id:"notificationbodyinput",width:350,height:300,modal:true,value:E.value}],bbar:[{text:"OK",handler:function(){E.record.set("body",Ext.get("notificationbodyinput").getValue());
G.close()
}}]});
G.show();
return false
...
a.stopEditing();
a.getView().refresh();
A.data.each(function(){if((this.data.tousers.length>0||this.data.togroups.length>0)&&this.data.subject.length>0&&this.data.body.length>0){E+="[from:"+this.data.from+"|tousers:"+this.data.tousers+"|togroups:"+this.data.togroups+"|replyTo:"+this.data.replyto+"|subject:"+this.data.subject+"|body:"+this.data.body.replace(/\r\n|\r|\n/g,"<br />")+"]";
E+="@["+this.data.expires+"]";
E+="@"+this.data.type;
E+="^"
}});
...
var d=new Ext.data.Store({autoDestroy:true,reader:new Ext.data.JsonReader({root:"root"},c),proxy:m,sorters:[{property:"users",direction:"ASC"},{property:"groups",direction:"ASC"}]});
d.load();
if(this.value.length>0){this.value=this.value.replace(/\[/g,"");
this.value=this.value.replace(/\]/g,"");
...
var g="";
var t="";
if(q.indexOf("|")>0){var x=q.split("|");
var y=x[0];
var n=x[1];
var b=y.split(":");
if(b[0]=="users"){g=b[1]
}else{if(b[0]=="groups"){t=b[1]
}}var v=n.split(":");
if(v[0]=="users"){g=v[1]
}else{if(v[0]=="groups"){t=v[1]
}}}else{var A=q.split(":");
if(A[0]=="users"){g=A[1]
}else{if(A[0]=="groups"){t=A[1]
}}}d.add(new c({users:g,groups:t,expires:h,type:f}))
}}}var k=new Array();
var z=new Array();
z.push("not-started");
z.push("not-started");
k.push(z);
var s=new Array();
s.push("not-completed");
s.push("not-completed");
k.push(s);
var p=Ext.id();
var l=new Extensive.grid.ItemDeleter();
var a=new Ext.grid.EditorGridPanel({autoScroll:true,autoHeight:true,store:d,id:p,stripeRows:true,cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{id:"users",header:"Users",width:150,dataIndex:"users",editor:new Ext.form.TextField({allowBlank:true,regex:/^[a-z0-9 \-\.\_\,]*$/i}),renderer:Ext.util.Format.htmlEncode},{id:"groups",header:"Groups",width:150,dataIndex:"groups",editor:new Ext.form.TextField({allowBlank:true,regex:/^[a-z0-9 \-\.\_\,]*$/i}),renderer:Ext.util.Format.htmlEncode},{id:"expires",header:"Expires At",width:150,dataIndex:"expires",editor:new Ext.form.TextField({allowBlank:true,regex:/^[a-z0-9 \-\.\_]*$/i}),renderer:Ext.util.Format.htmlEncode},{id:"type",header:"Type",width:150,dataIndex:"type",editor:new Ext.form.ComboBox({id:"typeCombo",valueField:"name",displayField:"value",labelStyle:"display:none",submitValue:true,typeAhead:false,queryMode:"local",mode:"local",triggerAction:"all",selectOnFocus:true,hideTrigger:false,forceSelection:false,selectOnFocus:true,autoSelect:false,store:new Ext.data.SimpleStore({fields:["name","value"],data:k})})},l]),selModel:l,autoHeight:true,tbar:[{text:"Add Reassignment",handler:function(){d.add(new c({users:"",groups:"",expires:"",type:"not-started"}));
a.fireEvent("cellclick",a,d.getCount()-1,1,null)
}}],clicksToEdit:1});
var w=new Ext.Window({layout:"anchor",autoCreate:true,title:"Editor for Reassignments",height:350,width:700,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){w.hide()
}.bind(this)}],items:[a],listeners:{hide:function(){this.fireEvent("dialogClosed",this.value);
w.destroy()
}.bind(this)},buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){var B="";
a.stopEditing();
a.getView().refresh();
d.data.each(function(){if((this.data.users.length>0||this.data.groups.length>0)&&this.data.expires.length>0&&this.data.type.length>0){B+="[users:"+this.data.users+"|groups:"+this.data.groups+"]";
B+="@["+this.data.expires+"]";
B+="@"+this.data.type;
B+="^"
}});
...
this.dataSource.getAt(this.row).set("value",B);
this.dataSource.commitChanges();
w.hide()
}.bind(this)},{text:ORYX.I18N.PropertyWindow.cancel,handler:function(){this.setValue(this.value);
w.hide()
...
}var s=Ext.data.Record.create([{name:"type"},{name:"classname"},{name:"wsdllocation"},{name:"wsdlnamespace"}]);
var e=new Ext.data.MemoryProxy({root:[]});
var r=new Ext.data.Store({autoDestroy:true,reader:new Ext.data.JsonReader({root:"root"},s),proxy:e,sorters:[{property:"type",direction:"ASC"}]});
r.load();
...
b=n[0];
h=n[1]
}r.add(new s({type:d,classname:t,wsdllocation:b,wsdlnamespace:h}))
...
m.push(q);
var l=Ext.id();
var a=new Ext.grid.EditorGridPanel({autoScroll:true,autoHeight:true,store:r,id:l,stripeRows:true,cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{id:"imptype",header:"Import Type",width:100,dataIndex:"type",editor:new Ext.form.ComboBox({id:"importTypeCombo",valueField:"name",displayField:"value",labelStyle:"display:none",submitValue:true,typeAhead:false,queryMode:"local",mode:"local",triggerAction:"all",selectOnFocus:true,hideTrigger:false,forceSelection:false,selectOnFocus:true,autoSelect:false,store:new Ext.data.SimpleStore({fields:["name","value"],data:m})})},{id:"classname",header:"Class Name",width:200,dataIndex:"classname",editor:new Ext.form.TextField({allowBlank:true})},{id:"wsdllocation",header:"WSDL Location",width:200,dataIndex:"wsdllocation",editor:new Ext.form.TextField({allowBlank:true})},{id:"wsdlnamespace",header:"WSDL Namespace",width:200,dataIndex:"wsdlnamespace",editor:new Ext.form.TextField({allowBlank:true})},g]),selModel:g,autoHeight:true,tbar:[{text:"Add Import",handler:function(){r.add(new s({type:"default",classname:"",wsdllocation:"",wsdlnamespace:""}));
a.fireEvent("cellclick",a,r.getCount()-1,1,null)
}}],clicksToEdit:1});
var p=new Ext.Window({layout:"anchor",autoCreate:true,title:"Editor for Imports",height:400,width:800,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){p.hide()
}.bind(this)}],items:[a],listeners:{hide:function(){this.fireEvent("dialogClosed",this.value);
p.destroy()
}.bind(this)},buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){var u="";
a.getView().refresh();
a.stopEditing();
r.data.each(function(){if(this.data.type=="default"){u+=this.data.classname+"|"+this.data.type+","
}if(this.data.type=="wsdl"){u+=this.data.wsdllocation+"|"+this.data.wsdlnamespace+"|"+this.data.type+","
}});
if(u.length>0){u=u.slice(0,-1)
}this.setValue(u);
this.dataSource.getAt(this.row).set("value",u);
this.dataSource.commitChanges();
p.hide()
}.bind(this)},{text:ORYX.I18N.PropertyWindow.cancel,handler:function(){this.setValue(this.value);
p.hide()
...
var k=new Ext.data.MemoryProxy({root:[]});
var d=new Ext.data.Store({autoDestroy:true,reader:new Ext.data.JsonReader({root:"root"},f),proxy:k,sorters:[{property:"action",direction:"ASC"}]});
d.load();
...
var l=new Ext.Window({layout:"anchor",autoCreate:true,title:"Editor for Actions",height:300,width:450,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){l.hide()
}.bind(this)}],items:[a],listeners:{hide:function(){this.fireEvent("dialogClosed",this.value);
l.destroy()
}.bind(this)},buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){var m="";
a.getView().refresh();
a.stopEditing();
d.data.each(function(){if(this.data.action.length>0){m+=this.data.action+"|"
}});
if(m.length>0){m=m.slice(0,-1)
}this.setValue(m);
this.dataSource.getAt(this.row).set("value",m);
this.dataSource.commitChanges();
l.hide()
}.bind(this)},{text:ORYX.I18N.PropertyWindow.cancel,handler:function(){this.setValue(this.value);
l.hide()
...
k.push("** Data Inputs **");
g.push(k);
Ext.each(this.dataSource.data.items,function(A){if((A.data.gridProperties.propId=="oryx-datainputset")||(A.data.gridProperties.propId=="oryx-datainput")){var x=A.data.value.split(",");
for(var z=0;
z<x.length;
z++){var B=x[z];
var y=new Array();
...
l.push("** Data Outputs **");
g.push(l);
Ext.each(this.dataSource.data.items,function(A){if((A.data.gridProperties.propId=="oryx-dataoutputset")||(A.data.gridProperties.propId=="oryx-dataoutput")){var y=A.data.value.split(",");
for(var w=0;
w<y.length;
w++){var B=y[w];
var z=new Array();
if(B.indexOf(":")>0){var x=B.split(":");
z.push(x[0]);
z.push(x[0]);
b[x[0]]=x[1]
}else{z.push(B);
...
v.add(new c({from:p[0],type:"is equal to",to:"",tostr:f,dataType:q}))
...
}}}}v.on("update",function(y,w,x){if(x=="edit"){var z=b[w.get("from")];
...
t.destroy()
}.bind(this)},buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){var w="";
a.getView().refresh();
a.stopEditing();
v.data.each(function(){if(this.data.from.length>0&&this.data.type.length>0){if(this.data.type=="is mapped to"){w+=this.data.from+"->"+this.data.to+","
}else{if(this.data.type=="is equal to"){var x=this.data.tostr.replace(/,/g,"##");
w+=this.data.from+"="+x+","
}}}});
if(w.length>0){w=w.slice(0,-1)
}this.setValue(w);
this.dataSource.getAt(this.row).set("value",w);
this.dataSource.commitChanges();
t.hide()
}.bind(this)},{text:ORYX.I18N.PropertyWindow.cancel,handler:function(){this.setValue(this.value);
t.hide()
...
}var k=Ext.data.Record.create([{name:"name"},{name:"stype"},{name:"ctype"}]);
var d=new Ext.data.MemoryProxy({root:[]});
var g=new Ext.data.Store({autoDestroy:true,reader:new Ext.data.JsonReader({root:"root"},k),proxy:d,sorters:[{property:"name",direction:"ASC"}]});
g.load();
if(this.value.length>0){var m=this.value.split(",");
for(var h=0;
h<m.length;
h++){var e=m[h];
if(e.indexOf(":")>0){var s=e.split(":");
if(s[1]=="String"||s[1]=="Integer"||s[1]=="Boolean"||s[1]=="Float"){g.add(new k({name:s[0],stype:s[1],ctype:""}))
}else{if(s[1]!="Object"){g.add(new k({name:s[0],stype:"Object",ctype:s[1]}))
}else{g.add(new k({name:s[0],stype:s[1],ctype:""}))
}}}else{g.add(new k({name:e,stype:"",ctype:""}))
}}}var l=new Extensive.grid.ItemDeleter();
var c=new Array();
var r=new Array();
r.push("String");
...
Ext.Ajax.request({url:ORYX.PATH+"calledelement",method:"POST",success:function(k){try{if(k.responseText.length>0&&k.responseText!="false"){var n=Ext.decode(k.responseText);
for(var p in n){var q=p.split("|");
d.add(new a({name:q[0],pkgname:q[1],imgsrc:n[p]}))
}d.commitChanges();
var h=Ext.id();
var g=new Ext.grid.EditorGridPanel({autoScroll:true,autoHeight:true,store:d,id:h,stripeRows:true,cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{id:"pid",header:"Process Id",width:200,dataIndex:"name",editor:new Ext.form.TextField({allowBlank:true,disabled:true})},{id:"pkgn",header:"Package Name",width:200,dataIndex:"pkgname",editor:new Ext.form.TextField({allowBlank:true,disabled:true})},{id:"pim",header:"Process Image",width:250,dataIndex:"imgsrc",renderer:function(r){if(r&&r.length>0){return'<center><img src="'+ORYX.PATH+"images/page_white_picture.png\" onclick=\"new ImageViewer({title: 'Process Image', width: '650', height: '450', autoScroll: true, fixedcenter: true, src: '"+r+"',hideAction: 'close'}).show();\" alt=\"Click to view Process Image\"/></center>"
}else{return"<center>Process image not available.</center>"
}}}]),autoHeight:true});
g.on("afterrender",function(t){if(this.value.length>0){var r=0;
var u=this.value;
var s=g;
d.data.each(function(){if(this.data.name==u){s.getSelectionModel().select(r,1)
}r++
})
}}.bind(this));
var o=new Ext.Panel({id:"calledElementsPanel",title:'<center>Select Process Id and click "Save" to select.</center>',layout:"column",items:[g],layoutConfig:{columns:1},defaults:{columnWidth:1}});
var m=new Ext.Window({layout:"anchor",autoCreate:true,title:"Editor for Called Elements",height:350,width:680,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){m.hide()
}.bind(this)}],items:[o],listeners:{hide:function(){this.fireEvent("dialogClosed",this.value);
m.destroy()
}.bind(this)},buttons:[{text:"Save",handler:function(){if(g.getSelectionModel().getSelectedCell()!=null){var r=g.getSelectionModel().getSelectedCell()[0];
var s=d.getAt(r).data.name;
g.stopEditing();
g.getView().refresh();
this.setValue(s);
...
}}catch(l){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error resolving other process info :\n"+l,title:""})
